...
public class JDBCRiver extends AbstractRiverComponent implements River {
	...
	private void createIndexIfNotExists()throws Exception{
		if(client.admin().indices().prepareExists(indexName).execute().actionGet().isExists()){
			...
			if(mappingES!=null && !"".equals(mappingES) && !verifyIfMappingExist()){
	...
	private boolean verifyIfMappingExist(){
		try{
			return client.admin().cluster().prepareState().execute().get().getState().getMetaData().getIndices().get(indexName).getMappings().containsKey(typeName);
		}catch(Exception e){
			logger.error("Impossible to check settings : " + e.getMessage());
			return false;
		}
